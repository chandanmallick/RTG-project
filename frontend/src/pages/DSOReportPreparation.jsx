...
export default function DSOReportPreparation({ reportType = "evening" }) {
  ...
  const defaultSicName = [user?.name, user?.designation]
    .filter(Boolean)
    .join(", ");
  ...
  const [limits, setLimits] = useState(
    Object.fromEntries(STATES.map((state) => [state, { ttc: "", atc: "" }])),
  );
    ...
    try {
      ...
      setMessage({
        ...
        text:
          ...
          error.message ||
          "Unable to load DSO report data.",
      });
    }
  };
  ...
  useEffect(() => {
    load();
  }, [reportDate, reportType]); // eslint-disable-line react-hooks/exhaustive-deps
  useEffect(() => {
  ...
  const executeProcess = async (overwrite = false) => {
    if (!file) {
      setMessage({
        type: "warning",
        text: "Select the SCADA PC Template workbook first.",
      });
      return;
    }
    setLoading(true);
    ...
    try {
      ...
      setMessage({
        type: "success",
        text: `DSO ${isEvening ? "Evening" : "Morning"} report processed and saved.`,
      });
    } catch (error) {
      setMessage({
        type: "error",
  ...
  const fetchCrmsOutageData = async () => {
    ...
    try {
      ...
      const summary =
        data.summary || data.report?.generation_outage_summary || {};
      if (summary.error) {
        setMessage({
          type: "error",
          text: `CRMS outage fetch failed: ${summary.error}`,
        });
      } else {
        setMessage({
          type: "success",
          text: `CRMS outage data fetched (${summary.row_count || 0} records) for ${reportDate}.`,
        });
      }
    } catch (error) {
      setMessage({
        ...
        text:
          error.response?.data?.detail ||
          error.message ||
          "CRMS outage data could not be fetched.",
      });
    } finally {
  ...
  const deleteReport = async () => {
    if (
      !window.confirm(
        `Delete the saved DSO ${isEvening ? "Evening" : "Morning"} report for ${reportDate}?`,
      )
    )
      return;
    try {
  ...
  const downloadImage = async () => {
    ...
    const canvas = await html2canvas(reportRef.current, {
      ...
      ignoreElements: (element) =>
        element.classList?.contains("dso-download-exclude"),
    });
  ...
  const downloadVisiblePdf = async () => {
    ...
    try {
      ...
      setMessage({
        type: "error",
        text: error.message || "Unable to generate the visible-page PDF.",
      });
    }
  };
  ...
  const udItems = STATES.map((state) => ({
    state,
    value: stateRows[state]?.ud_at_max_frequency_mw,
  })).filter((item) => Number(item.value) > 0);
  const tableSx = {
    width: "100%",
  ...
  const openReportEditor = () => {
    const revivedDetails =
      thermal.revived_details ||
      (thermal.units?.revived || [])
        .map((item) => `${item.unit} (${number(item.capacity_mw, 0)} MW)`)
        .join("\n");
    const outageDetails =
      thermal.outage_details ||
      (thermal.units?.outage || [])
        .map(
          (item) =>
            `${item.unit} (${number(item.capacity_mw, 0)} MW)${item.reason ? ` [${item.reason}]` : ""}`,
        )
        .join("\n");
    setEditDraft({
      ...
      states: Object.fromEntries(
        STATES.map((state) => [state, { ...(stateRows[state] || {}) }]),
      ),
  ...
  const saveReportEdit = async () => {
    ...
    try {
      ...
      setMessage({
        type: "success",
        text: "All report section changes were saved.",
      });
    } catch (error) {
      setMessage({
        type: "error",
  ...
  const setDraftState = (state, key, value) => {
    setEditDraft((current) => ({
      ...
      states: {
        ...current.states,
        [state]: { ...current.states[state], [key]: value },
      },
    }));
  };
  ...
  return (
    <AppShell>
        ...
        >
          ...
          >
            <Box>
              ...
              <Typography sx={{ fontSize: 11, opacity: 0.9, mt: 0.35 }}>
                Important ER Grid information from 00:00 hrs through 17:00 hrs.
              </Typography>
            </Box>
            <Stack
              direction="row"
            ...
            >
              <Box sx={{ textAlign: "right", mr: 0.5 }}>
                <Typography
                  sx={{ fontSize: 9, fontWeight: 900, lineHeight: 1 }}
                >
                  REPORT DATE
                </Typography>
              ...
              <Button
                disabled={!canWrite || crmsLoading}
                onClick={fetchCrmsOutageData}
                startIcon={
                  crmsLoading ? (
              ...
              >
                {crmsLoading ? "Fetching CRMS…" : "Fetch CRMS Outage"}
              </Button>
              {report && (
                <Button
                  disabled={!canWrite}
              ...
              {report && (
                <Button
                  href={API.dsoReportExcelUrl(reportType, reportDate)}
                  startIcon={<Download size={16} />}
                  variant="outlined"
                  sx={{ borderColor: "#fff", color: "#fff", fontWeight: 900 }}
                >
                  Download Excel
                </Button>
              )}
        ...
        <Card sx={{ mb: 2 }}>
          ...
          <Box
            sx={{
              ...
              gridTemplateColumns: {
                xs: "1fr",
                xl: "190px minmax(280px,1fr) minmax(220px,.6fr) auto",
              },
              gap: 1.5,
              alignItems: "center",
            }}
          >
            ...
            >
              {file?.name ||
                `Upload SCADA PC Template (${isEvening ? "00:00 to 16:59 hrs" : "report window"})`}
              <input
                hidden
                type="file"
                accept=".xlsx,.xlsm"
            ...
            <TextField
              ...
              label="Name of SIC"
              value={sicName}
              onChange={(event) => setSicName(event.target.value)}
              size="small"
              helperText="Automatically populated from login"
            />
            <Button
              onClick={process}
              ...
              sx={{ height: 40, bgcolor: "#0068D9", fontWeight: 900, px: 3 }}
            >
          ...
          <TextField
            disabled={!canWrite}
            label="Important Events (FTC / GD / GI / Load crash etc.)"
            value={events}
          ...
          {report?.input_summary && (
            <Typography sx={{ mt: 1, fontSize: 11, color: "#64748B" }}>
              Processed {report.input_summary.processed_rows} rows from “
              {report.input_summary.sheet}”, {report.input_summary.first_time}–
              {report.input_summary.last_time}. Uploaded source rows are not
              stored.
            </Typography>
          )}
        </Card>
        ...
        {!report ? (
          <Card>
            <Box sx={{ py: 6, textAlign: "center" }}>
              <CalendarDays size={34} color="#7A96B8" />
              <Typography sx={{ mt: 1, fontWeight: 900 }}>
                No processed report for this date
              </Typography>
              <Typography sx={{ color: "#64748B", fontSize: 12 }}>
                Upload the SCADA workbook to prepare and save the report.
              </Typography>
            </Box>
          </Card>
        ) : (
          <Box ref={reportRef}>
            <Card sx={{ mb: 2 }}>
              ...
              >
                Important information update for ER Grid (since 00:00 hrs to
                17:00 hrs) for {reportDate.split("-").reverse().join("-")}
              </Typography>
              <Box
                sx={{
                  display: "grid",
              ...
              >
                <Box sx={{ overflowX: "auto" }}>
                  <Box
                    component="table"
                    sx={{ ...tableSx, minWidth: { xs: 480, lg: 0 } }}
                  >
                    <thead>
                      <tr>
                        <th></th>
                    ...
                    <tbody>
                      <tr>
                        ...
                        <td>{number(metrics.max_demand_mw, 0)}</td>
                        <td>{metrics.max_demand_time}</td>
                      </tr>
                      <tr>
                        <td style={{ textAlign: "left", fontWeight: 900 }}>
                          Min demand met (MW)
                        </td>
                        <td>{number(metrics.min_demand_mw, 0)}</td>
                        <td>{metrics.min_demand_time}</td>
                      </tr>
                      <tr>
                        <td style={{ textAlign: "left", fontWeight: 900 }}>
                          Max freq. (Hz)
                        </td>
                        <td>{number(metrics.max_frequency_hz, 3)}</td>
                        <td>{metrics.max_frequency_time}</td>
                      </tr>
                      <tr>
                        <td style={{ textAlign: "left", fontWeight: 900 }}>
                          Min freq. (Hz)
                        </td>
                        <td>{number(metrics.min_frequency_hz, 3)}</td>
                        <td>{metrics.min_frequency_time}</td>
                      </tr>
                    </tbody>
                  </Box>
                </Box>
                <Box sx={{ overflowX: "auto" }}>
                  <Box
                    component="table"
                    sx={{ ...tableSx, minWidth: { xs: 280, lg: 0 } }}
                  >
                    <thead>
                      <tr>
                        <th>Frequency</th>
                    ...
                    <tbody>
                      <tr>
                        ...
                        <td>{number(frequency.above_50_05_pct, 2)}</td>
                      </tr>
                      <tr>
                        <td>
                          <b>within band</b>
                        </td>
                        <td>{number(frequency.within_band_pct, 2)}</td>
                      </tr>
                      <tr>
                        <td>
                          <b>&lt;49.9</b>
                        </td>
                        <td>{number(frequency.below_49_9_pct, 2)}</td>
                      </tr>
                    </tbody>
                  </Box>
                </Box>
            ...
            >
              <Card sx={{ p: 0, overflow: "hidden" }}>
                ...
                <Box sx={{ overflowX: "auto", p: 1.5 }}>
                  <Box
                    component="table"
                    sx={{ ...tableSx, minWidth: { xs: 560, xl: 0 } }}
                  >
                    <thead>
                      <tr>
                        <th>Revived capacity (MW)</th>
                        <th>Outage capacity (MW)</th>
                        <th>Net Capacity addition (+)/ reduction (-) (MW)</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr>
                        <td>
                          <b>{number(thermal.revived_capacity_mw, 0)}</b>
                        </td>
                        <td>
                          <b>{number(thermal.outage_capacity_mw, 0)}</b>
                        </td>
                        <td>
                          <b>{number(thermal.net_capacity_change_mw, 0)}</b>
                        </td>
                      </tr>
                      <tr>
                        <td style={{ whiteSpace: "pre-wrap" }}>
                          {thermal.revived_details ||
                            ((thermal.units?.revived || []).length
                              ? thermal.units.revived.map((item) => (
                                  <Box
                                    key={`${item.unit}-${item.capacity_mw}`}
                                    sx={{ mb: 0.7 }}
                                  >
                                    {item.unit} ({number(item.capacity_mw, 0)}{" "}
                                    MW)
                                  </Box>
                                ))
                              : "NIL")}
                        ...
                        <td style={{ whiteSpace: "pre-wrap" }}>
                          {thermal.outage_details ||
                            ((thermal.units?.outage || []).length
                              ? thermal.units.outage.map((item) => (
                                  <Box
                                    key={`${item.unit}-${item.capacity_mw}`}
                                    sx={{ mb: 0.7 }}
                                  >
                                    {item.unit} ({number(item.capacity_mw, 0)}{" "}
                                    MW){item.reason ? ` [${item.reason}]` : ""}
                                  </Box>
                                ))
                              : "NIL")}
                        </td>
              ...
              <Card sx={{ p: 0, overflow: "hidden" }}>
                ...
                <Box sx={{ overflowX: "auto", p: 1.5 }}>
                  <Box
                    component="table"
                    sx={{ ...tableSx, minWidth: { xs: 720, xl: 0 } }}
                  >
                    <thead>
                      <tr>
                        <th>STATE</th>
                    ...
                    <tbody>
                      {[
                        [
                          "TTC LIMIT",
                          (row) =>
                            row.ttc_limit_mw === null ||
                            row.ttc_limit_mw === undefined
                              ? "—"
                              : `${number(row.ttc_limit_mw, 0)} (0-24 Hrs.)`,
                        ],
                        [
                          "ATC LIMIT",
                          (row) =>
                            row.atc_limit_mw === null ||
                            row.atc_limit_mw === undefined
                              ? "—"
                              : `${number(row.atc_limit_mw, 0)} (0-24 Hrs.)`,
                        ],
                        [
                          "MAX SCHEDULE",
                          (row) =>
                            row.max_schedule_mw === null ||
                            row.max_schedule_mw === undefined ? (
                              "—"
                            ) : (
                              <>
                                {number(row.max_schedule_mw, 0)}
                                <br />
                                <small>({row.max_schedule_time})</small>
                              </>
                            ),
                        ],
                        [
                          "MAX ACTUAL",
                          (row) =>
                            row.max_actual_mw === null ||
                            row.max_actual_mw === undefined ? (
                              "—"
                            ) : (
                              <>
                                {number(row.max_actual_mw, 0)}
                                <br />
                                <small>({row.max_actual_time})</small>
                              </>
                            ),
                        ],
                        [
                          "VIOLATION",
                          (row) =>
                            row.atc_violation_mw === null ||
                            row.atc_violation_mw === undefined
                              ? "NIL"
                              : `${number(row.atc_violation_mw, 0)} MW`,
                        ],
                      ].map(([label, render]) => (
                        <tr key={label}>
            ...
            <Box
              sx={{
                display: "grid",
                gridTemplateColumns: { xs: "1fr", xl: "1.12fr .88fr" },
                gap: 2,
                mb: 2,
              }}
            >
              <Card sx={{ p: 0, overflow: "hidden" }}>
                ...
                <Typography
                  ...
                  sx={{
                    px: 1.7,
                    pt: 0.8,
                    color: generationOutageMeta.error ? "#B91C1C" : "#64748B",
                    fontSize: 11.5,
                  }}
                >
                  Source:{" "}
                  {generationOutageMeta.source_name ||
                    "CRMS GenOutagesHistoryData"}
                  {generationOutageMeta.error
                    ? ` · Fetch failed: ${String(generationOutageMeta.error).slice(0, 140)}`
                    : ` · ${generationOutageMeta.row_count || 0} outage records`}
                </Typography>
                <Box sx={{ overflowX: "auto", p: 1.5 }}>
                  <Box component="table" sx={{ ...tableSx, minWidth: 520 }}>
                    <thead>
                      <tr>
                        <th>Fuel</th>
                        <th>Planned outage (MW)</th>
                        <th>Forced outage (MW)</th>
                        <th>Total under outage (MW)</th>
                      </tr>
                    </thead>
                    <tbody>
                      {["THERMAL", "HYDRO"].map((fuel) => {
                        ...
                        return (
                          <tr key={fuel}>
                            <td style={{ fontWeight: 900 }}>{fuel}</td>
                            <td>
                              {number(item.planned_mw, 0)}
                              {item.planned_units
                                ? ` (${item.planned_units} units)`
                                : ""}
                            </td>
                            <td>
                              {number(item.forced_mw, 0)}
                              {item.forced_units
                                ? ` (${item.forced_units} units)`
                                : ""}
                            </td>
              ...
              <Card sx={{ p: 0, overflow: "hidden" }}>
                ...
                >
                  Major OD/UD by states/generators:
                </Typography>
            ...
            <Card sx={{ p: 0, mb: 2, overflow: "hidden" }}>
              ...
              >
                Important Events (FTC/GD/GI/Load crash etc.):
              </Typography>
              <Typography
                sx={{
                  p: 1.7,
              ...
              >
                {report.important_events || "NIL"}
              </Typography>
            </Card>

            <Box sx={{ px: 1, pb: 1 }}>
              <Typography sx={{ fontSize: 12, fontWeight: 900 }}>
                {report.signoff_regards || "Regards"}
              </Typography>
              <Typography sx={{ fontSize: 13 }}>
                {report.signoff_name || "Ashoke Kumar Basak, SIC ERLDC"}
              </Typography>
            </Box>
          </Box>
        )}
      ...
      <Dialog
        open={overwriteOpen}
        onClose={() => !loading && setOverwriteOpen(false)}
        maxWidth="sm"
        fullWidth
      >
        <DialogTitle sx={{ fontWeight: 900 }}>
          Overwrite saved report?
        </DialogTitle>
        <DialogContent dividers>
          <Typography>
            A report is already saved for{" "}
            {reportDate.split("-").reverse().join("-")}. The new workbook will
            replace the older processed report.
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setOverwriteOpen(false)} disabled={loading}>
            Cancel
          </Button>
          <Button
      ...
      >
        <DialogTitle sx={{ fontWeight: 900 }}>
          TTC / ATC Limit Master (+ve Import, -ve Export)
        </DialogTitle>
        <DialogContent dividers>
          <Box
            sx={{
          ...
          >
            {STATES.map((state) => (
              <Card key={state} sx={{ p: 1.5 }}>
                <Typography sx={{ fontWeight: 900, color: "#0057B7", mb: 1 }}>
                  {state}
                </Typography>
                <Stack direction="row" spacing={1}>
                  {["ttc", "atc"].map((kind) => (
                    <TextField
                      key={kind}
                      label={`${kind.toUpperCase()} Limit (MW)`}
                      type="number"
                      size="small"
                      value={limits[state]?.[kind] ?? ""}
                      onChange={(event) =>
                        setLimits((current) => ({
                          ...current,
                          [state]: {
            ...
            <Stack spacing={2}>
              <Card>
                <Typography sx={{ mb: 1.5, fontWeight: 900, color: "#0057B7" }}>
                  1. Demand and Frequency Summary
                </Typography>
                <Box
                  sx={{
                    display: "grid",
                ...
                >
                  {[
                    ...
                    <TextField
                      ...
                      InputLabelProps={{ shrink: true }}
                      inputProps={
                        type === "number" ? { step: "any" } : undefined
                      }
                    />
                  ))}
                </Box>
                ...
                >
                  {[
                    ...
                    <TextField
                      ...
                      type="number"
                      size="small"
                      value={editDraft.frequency_distribution?.[key] ?? ""}
                      onChange={(event) =>
                        setDraftSection(
              ...
              <Card>
                <Typography sx={{ mb: 1.5, fontWeight: 900, color: "#0057B7" }}>
                  2. Thermal Generation Availability
                </Typography>
                <Box
                  sx={{
                    display: "grid",
                ...
                >
                  {[
                    ...
                    <TextField
                      ...
                      type="number"
                      size="small"
                      value={editDraft.thermal_availability?.[key] ?? ""}
                      onChange={(event) =>
                        setDraftSection(
                          "thermal_availability",
                          key,
                ...
                >
                  <TextField
                    label="Revived unit details"
                    multiline
                    minRows={4}
                    value={
                      editDraft.thermal_availability?.revived_details || ""
                    }
                    onChange={(event) =>
                      setDraftSection(
                        "thermal_availability",
                  ...
                  />
                  <TextField
                    label="Outage unit details and reasons"
                    multiline
                    minRows={4}
                    value={editDraft.thermal_availability?.outage_details || ""}
                    onChange={(event) =>
                      setDraftSection(
                        "thermal_availability",
                        "outage_details",
              ...
              <Card>
                <Typography sx={{ mb: 1.5, fontWeight: 900, color: "#0057B7" }}>
                  3. State TTC/ATC, Schedule, Actual and OD/UD
                </Typography>
                <Box
                  sx={{
                    display: "grid",
                ...
                >
                  {STATES.map((state) => (
                    ...
                    >
                      <Typography
                        sx={{ mb: 1.2, fontWeight: 900, color: "#006845" }}
                      >
                        {state}
                      </Typography>
                      <Box
                      ...
                      >
                        {[
                          ...
                          [
                            "OD at minimum frequency (MW)",
                            "od_at_min_frequency_mw",
                            "number",
                          ],
                          [
                            "UD at maximum frequency (MW)",
                            "ud_at_max_frequency_mw",
                            "number",
                          ],
                        ].map(([label, key, type]) => (
                          <TextField
                            ...
                            onChange={(event) =>
                              setDraftState(state, key, event.target.value)
                            }
                            InputLabelProps={{ shrink: true }}
                            inputProps={
                              type === "number" ? { step: "any" } : undefined
              ...
              <Card>
                ...
                <Stack spacing={1.3}>
                  <TextField
                    ...
                    multiline
                    minRows={2}
                    value={editDraft.major_od_text || ""}
                    onChange={(event) =>
                      setEditDraft((current) => ({
                        ...current,
                        major_od_text: event.target.value,
                      }))
                    }
                  />
                  <TextField
                    ...
                    multiline
                    minRows={2}
                    value={editDraft.major_ud_text || ""}
                    onChange={(event) =>
                      setEditDraft((current) => ({
                        ...current,
                        major_ud_text: event.target.value,
                      }))
                    }
                  />
                </Stack>
              ...
              <Card>
                <Typography sx={{ mb: 1.5, fontWeight: 900, color: "#0057B7" }}>
                  5. Important Events and Sign-off
                </Typography>
                <TextField
                  label="Important Events (FTC/GD/GI/Load crash etc.)"
                  multiline
                  minRows={3}
                  fullWidth
                  value={editDraft.important_events || ""}
                  onChange={(event) =>
                    setEditDraft((current) => ({
                      ...current,
                      important_events: event.target.value,
                    }))
                  }
                />
                <Box
                ...
                >
                  <TextField
                    label="Regards / closing text"
                    value={editDraft.signoff_regards || ""}
                    onChange={(event) =>
                      setEditDraft((current) => ({
                        ...current,
                        signoff_regards: event.target.value,
                      }))
                    }
                  />
                  <TextField
                    label="Name of SIC and designation"
                    value={editDraft.signoff_name || ""}
                    onChange={(event) =>
                      setEditDraft((current) => ({
                        ...current,
                        signoff_name: event.target.value,
                      }))
                    }
                  />
                </Box>
